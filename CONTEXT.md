...
Secant is a Harness-oriented meta-orchestrator: it reaches an outcome by routing a Workflow Bundle's Steps through an external coding Harness against
a Workspace. The name is decided in [ADR 0028](./docs/adr/0028-adopt-secant-as-the-product-package-and-command-name.md); older tickets and ADRs
say Crucible, the working name it replaced.
...
- **Workspace** — the resolved absolute directory a **Run** executes against, taken from the directory where Secant was launched. It is a value
...
- **Run** — one execution of one **Workflow Bundle** against one **Workspace** through one **Harness**. See the
  [Secant Run lifecycle](./docs/glossary/secant-run-lifecycle.md) cluster for everything inside a Run.
- **Catalog Entry** — the local record that one exact **Workflow Bundle** identity and digest is installed, plus its advisory **Bundle origin**.
  Installing creates one; uninstalling removes it with Secant's managed Bundle bytes.
...
- **External Workflow Bundle** — a **Workflow Bundle** supplied outside Secant's built-in set. It follows the same package and execution
...
- **Routing** — the ordered arrangement of **Steps** a **Workflow Bundle** declares over Secant's **step kinds**, with contiguous spans optionally
  ...
  composes — see **Composition check** in the [Secant Run lifecycle](./docs/glossary/secant-run-lifecycle.md) cluster.
- **Step kind** — one of Secant's four built-in **Step** contracts: **Agent step**, **Interactive agent step**, **Human Gate**, or **Command step**.
  ...
  preconditions, its capability needs, which attempt outcomes it may retry, and how an **Indeterminate attempt** is reconciled. Secant owns the
  set; a **Workflow Bundle** supplies content, parameters, and optional **Workspace prerequisites**, never its own step implementation.
- **Workspace prerequisite** — one of Secant's closed semantic facts about a **Workspace** that an authored **Step** may require and **Preflight**
...
- **Proof Bundle** — the role held by a maintained **External Workflow Bundle** whose purpose is to keep Secant's step vocabulary honest. It
...
- **Test Repair Workflow** — the first **Proof Bundle**. Given a path to a failing test, it drives that test to green through bounded **Step
  ...
  ordering is a Bundle contract, not a Git invariant Secant enforces around the **Harness**.
- **Harness** — an external coding-agent runtime Secant can select for workflow execution. The shipped Harness portfolio is product scope rather
...
- **Projection** — a disposable bounded task view over canonical Secant truth, optionally combined with an explicitly separate live Harness
...
- **Action Offer** — a typed, state-bound opportunity projected for one exact semantic target, including the input it accepts and evidence when it
  ...
  capability flag.
- **Operation** — Secant's durable admission and application/control result for one user intent, correlated by a caller-generated id. It does
...
- [Secant Run lifecycle](./docs/glossary/secant-run-lifecycle.md) — Runs, Steps, Step Attempts, Turns, Run Artifacts, Harness Sessions, Human
