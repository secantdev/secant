...
- Leaves a stub CLI entry answering `--help` and `--version` so the installed-package smoke passes, keeps the package name and bin unchanged
  pending [the naming decision](https://github.com/DevFlow-HQ/devflow-cli/issues/24), and replaces the README with an in-development notice.
