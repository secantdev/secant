...
| GitHub organization | `secantdev`         | The bare `secant` login is a dormant user account from 2017, so the organization is qualified. `secant-dev` was first reserved as an empty placeholder on 2026-09-06, but deleting a placeholder locks its name for 90 days, so the unhyphenated `secantdev` was taken instead (amended 2026-09-07). `DevFlow-HQ` was renamed to `secantdev` on 2026-09-07 and `DevFlow-HQ` re-registered empty so nobody can inherit its redirects. |
...
2. An authenticated `npm publish --dry-run` for `secant`. The npm user and organization pages returned access errors during research, so the npm
   scope is unverified. The organization and repository rename that this gate also named was completed on 2026-09-07. The dry-run was run on
   2026-09-07: the bare `secant` name is unpublishable (npm blocks it as too similar to `recast`), so the scoped fallback `@secantdev/secant` was
   reserved with a `0.0.0` placeholder under a newly created `secantdev` npm org (amended 2026-09-07).
...
deletes legacy DevFlow. That commit renames the package name, the bin, `CONTEXT.md`, `README.md`, and agent guidance to Secant, so no target code is
ever written under the DevFlow or Crucible names. The organization and repository were renamed ahead of the cut on 2026-09-07, as the
[sequencing decision](https://github.com/DevFlow-HQ/devflow-cli/issues/20) placed them (amended 2026-09-07). Nothing is published from that commit. Existing ADRs and closed
