...
`running` Run becomes `halted` with the `indeterminate` marker as before, while a derived-`blocked` Run **stays `blocked`** with its ownership
released, because nothing was cut off and the pending checkpoint is still true. Fencing, not the probe, is what makes ownership safe: acquiring a
...
or another, and quitting halts every live Run after one confirmation naming the count. Rejected: keeping the claim as an advisory launch warning (a
