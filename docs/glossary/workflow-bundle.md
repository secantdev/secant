...
This cluster defines Secant's portable workflow package. It fixes what a Bundle may declare, how all Bundle sources enter the local Catalog, and
...
- **Installed Bundle** — the exact `.wfb` bytes copied into Secant's managed store and named by one **Catalog Entry**. Original authoring folders
  and imported files are not Installed Bundles and remain outside Secant's ownership. _Avoid_: Archive copy, Bundle Snapshot.
- **Bundle origin** — advisory Catalog metadata describing where Secant obtained an Installed Bundle: an app release (recorded with the Secant
...
- **Execution summary** — Secant's generated account of the authority an Installed Bundle can exercise on the selected platform, shown before
...
- The package format sets no universal size ceiling. Each Secant installation applies configurable budgets to input bytes, expanded bytes, and
...
- Steps distinguish `{"asset":"path"}` from `{"artifact":"name"}`. Only explicitly referenced assets are materialized, in Secant-owned
...
- Commands run as the current OS user. Secant promises no command sandbox: commands and Harness actions may access the user's files, environment,
...
- Built-in, local-build, local-file, and future portal Bundles share one ingestion and runtime contract. Versions install side by side; interactive
  selection defaults to the highest stable installed version, prereleases require explicit selection, and Secant never auto-updates.
- External Bundles install untrusted. Before the first attempted Run for a digest, Secant presents its generated Execution summary and asks once;
...
- Successfully uninstalling an External Workflow Bundle removes only Secant's managed `.wfb`, Catalog Entry, and Trust grant. It preserves the
