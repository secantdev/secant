...
| `unresponsive`           | swallows SIGTERM → force-kill, reported escalated (every OS; on Windows a hidden console child survives the graceful `taskkill /T` stage the same way) | a real `claude` honours SIGTERM                               |
